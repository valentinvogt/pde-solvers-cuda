...
void convolve_sigma_add_f(zisa::array_view<Scalar, 2> dst,
  ...
  if (!(memory_dst == memory_src && memory_src == memory_sigma)) {
    std::cerr << "Convolve sigma add f: Inputs must be located on the same hardware\n";
    exit(1);
  }
  if (dst.shape() != src.shape()) {
    std::cerr
        << "Convolve sigma add f: Input and output array must have the same shape\n";
    exit(1);
  }
  if (dst.shape(0) != sigma.shape(0) + 1 && dst.shape(1) != sigma.shape(1)) {
    std::cerr
        << "Convolve sigma add f: Input array has to be one element bigger than sigma array\n";
    exit(1);
  }
